...
public class TestPythonServer {
	...
	public void setUp() throws IOException {
		...
		gatewayServer.start();
		pythonEntryPoint = (IPythonEntryPoint) gatewayServer
	...
	@Test
	public void testRemoveZoneFromRoom() {
		IZone innerZone = pythonEntryPoint.createZone("HVAC Zone 1", ZoneType.HVAC.getValue());
		innerZone.setHvacType(HvacType.INTERIOR.getValue());
		IZone outerZone = pythonEntryPoint.createZone("HVAC Zone 2", ZoneType.HVAC.getValue());
		outerZone.setHvacType(HvacType.PERIMETER.getValue());
	...
	public void testGetMeterMeasureByDate() {
		...
		Assert.assertEquals(meter
				.getMeterMeasureByDate(startDate.format(formatter), startDate.plusDays(2).format(formatter)).size(), 3);
	...
	public void testRemoveSensorFromRoom() {
		...
		IBinaryMeasure measurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(measure, "Binary");
		IRoom room = pythonEntryPoint.createRoom(measurement, "Room 001", RoomType.OFFICE.getValue(), "hei.ies.ies");
		ISensor sensor = pythonEntryPoint.createSensor("TMP 01", SensorMeasure.HUMIDITY.getValue(),
				MeasurementUnit.RELATIVE_HUMIDITY.getValue(), SensorMeasureType.THERMO_COUPLE_TYPE_A.getValue(), 900);
		room.addTransducer(sensor);
	...
	public void testCreateOpenSpace() {
		...
		IBinaryMeasure measurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(measure, "Binary");
		IOpenSpace openSpace = pythonEntryPoint.createOpenSpace(measurement, "Hall 001",
				OpenSpaceType.CORRIDOR.getValue(), null);
	...
	public void testCreateOpenSpaceWithSensors() {
		...
		ISensor tempSensor = pythonEntryPoint.createSensor("TMP 01", SensorMeasure.TEMPERATURE.getValue(),
				MeasurementUnit.DEGREE_CELSIUS.getValue(), SensorMeasureType.THERMO_COUPLE_TYPE_A.getValue(), 900);
		String description = "Measures temperature";
		tempSensor.addMetaData("description", description);
		ISensor co2Sensor = pythonEntryPoint.createSensor("CO2 01", SensorMeasure.CARBON_DIOXIDE.getValue(),
				MeasurementUnit.PARTS_PER_MILLION.getValue(), SensorMeasureType.THERMO_COUPLE_TYPE_A.getValue(), 900);
		...
		Assert.assertEquals(openSpace.getTransducer(tempSensor.getName()).getMetaData().get("description"),
				description);
	...
	public void testCreateOpenSpaceWithDuplicateSensors() {
		...
		ISensor tempSensor = pythonEntryPoint.createSensor("TMP 01", SensorMeasure.TEMPERATURE.getValue(),
				MeasurementUnit.DEGREE_CELSIUS.getValue(), SensorMeasureType.THERMO_COUPLE_TYPE_A.getValue(), 900);
		ISensor tempSensor2 = pythonEntryPoint.createSensor("TMP 01", SensorMeasure.TEMPERATURE.getValue(),
				MeasurementUnit.DEGREE_CELSIUS.getValue(), SensorMeasureType.THERMO_COUPLE_TYPE_B.getValue(), 1200);
	...
	public void testCreateFloorWithRoom() {
		...
		IRoom room = pythonEntryPoint.createRoom(roomMeasurement, "Room 001", RoomType.OFFICE.getValue(),
				"hei.ies.ies");
		...
		IFloor floor = pythonEntryPoint.createFloor(floorMeasurement, 1, FloorType.REGULAR.getValue(), floorMeasurement,
				"First floor of the building", room, null);
	...
	public void testCreateFloorWithRoomAndOpenSpace() {
		...
		IOpenSpace openSpace = pythonEntryPoint.createOpenSpace(roomMeasurement, "Hall 001",
				OpenSpaceType.CORRIDOR.getValue(), null);

		IMeasure floorSize = pythonEntryPoint.createMeasure(MeasurementUnit.SQUARE_METERS.getValue(), 150);
		IBinaryMeasure floorMeasurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(floorSize, "Binary");
		IFloor floor = pythonEntryPoint.createFloor(floorMeasurement, 1, FloorType.REGULAR.getValue(), floorMeasurement,
				"First floor of the building", room, openSpace);
	...
	@Test
	public void testCreateFloorWithRoomHavingMeter() {
		IMeasure measure = pythonEntryPoint.createMeasure(MeasurementUnit.SQUARE_METERS.getValue(), 20);
		IBinaryMeasure measurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(measure, "Binary");
		IRoom room = pythonEntryPoint.createRoom(measurement, "Room 001", RoomType.OFFICE.getValue(), "hei.ies.ies");
	...
	public void testCreateFloorWithRoomHavingMeterAndSensor() {
		...
		IMeasure floorSize = pythonEntryPoint.createMeasure(MeasurementUnit.SQUARE_METERS.getValue(), 150);
		IBinaryMeasure floorMeasurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(floorSize, "Binary");
		IFloor floor = pythonEntryPoint.createFloor(floorMeasurement, 1, FloorType.BASEMENT.getValue(),
				floorMeasurement, "First floor of the building", room, null);

		Assert.assertEquals(floor.getRoomByName(room.getName()).getTransducer(sensor.getName()).toString(),
				sensor.toString());
		Assert.assertEquals(floor.getRoomByName(room.getName()).getMeter().toString(), meter.toString());
	...
	public void testCreateWeatherStation() {
		...
		Assert.assertEquals(weatherStation.getWeatherData(null).get(0).getData().getMeasurementUnit(),
				MeasurementUnit.RELATIVE_HUMIDITY.getValue());
	}

	@Test
	public void testBuildingEnvelope() {
		IMeasure thermalTransmittanceMeasure = pythonEntryPoint
				.createMeasure(MeasurementUnit.WATTS_PER_SQUARE_METER_KELVIN.getValue(), 10);
		IBinaryMeasure ttmMeasurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(thermalTransmittanceMeasure,
				"Binary");

		IMeasure densityMeasure = pythonEntryPoint.createMeasure(MeasurementUnit.KILOGRAM_PER_CUBIC_METER.getValue(),
				7.8);
		IBinaryMeasure densityMeasurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(densityMeasure,
				"Binary");

		IMeasure heatCapacityMeasure = pythonEntryPoint.createMeasure(MeasurementUnit.JOULES_PER_KELVIN.getValue(),
				12.89);
		IBinaryMeasure hcMeasurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(heatCapacityMeasure,
				"Binary");

		IMeasure thermalResistanceMeasure = pythonEntryPoint
				.createMeasure(MeasurementUnit.SQUARE_METERS_KELVIN_PER_WATTS.getValue(), 15.898);
		IBinaryMeasure trMeasurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(thermalResistanceMeasure,
				"Binary");

		//Create material
		IMaterial material = pythonEntryPoint.createMaterial("Roofing Material", MaterialType.ROOF_ALUMINIUM.getValue(),
				densityMeasurement, ttmMeasurement, hcMeasurement, trMeasurement, 0.0387);
		...
		IBinaryMeasure thicknessMeasurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(thicknessMeasure,
				"Binary");

		//Create roofing layer
		ILayer layer = pythonEntryPoint.createLayer(lengthMeasurement, lengthMeasurement, thicknessMeasurement,
				material);
		...
		Assert.assertEquals(envelope.getCoverByUID(cover.getUID()).getLayerByUID(layer.getUID()).toString(),
				layer.toString());
		Assert.assertEquals(
				envelope.getCoverByUID(cover.getUID()).getLayerByUID(layer.getUID()).getMaterial().toString(),
				material.toString());
	}
	...
	public void testCreateBuildingWithOneFloor() {
		...
		IBinaryMeasure buildingFloorArea = (IBinaryMeasure) pythonEntryPoint.createMeasurement(floorAreaMeasure,
				"Binary");
		IPoint coordinates = pythonEntryPoint.createCoordinates(45.4967765, -73.5806159);
		IAddress address = pythonEntryPoint.createAddress("Montreal", "1400 de Maisonneuve Blvd. W.", "QC", "H3G 1M8",
				"Canada", coordinates);
	...
	public void testCreateBuildingWithRoofingCover() {
		...
		Assert.assertEquals(building.getEnvelope().getCoverByUID(cover.getUID()).getLayerByUID(layer.getUID())
				.getHeight().toString(), lengthMeasurement.toString());
		Assert.assertEquals(building.getEnvelope().getCoverByUID(cover.getUID()).getCoverType(),
				CoverType.ROOF.getValue());
		Assert.assertEquals(building.getEnvelope().getCoverByUID(cover.getUID()).getLayerByUID(layer.getUID())
				.getMaterial().toString(), material.toString());
	}
	...
	public void testCreateBuildingWithFloors() {
		...
		IOpenSpace openSpace = pythonEntryPoint.createOpenSpace(osMeasurement, "Hall 001",
				OpenSpaceType.CORRIDOR.getValue(), null);
		ISensor co2Sensor = pythonEntryPoint.createSensor("CO2 01", SensorMeasure.CARBON_DIOXIDE.getValue(),
				MeasurementUnit.PARTS_PER_MILLION.getValue(), SensorMeasureType.THERMO_COUPLE_TYPE_A.getValue(), 900);
		openSpace.addTransducer(co2Sensor);
		...
		IMeasure floorSize = pythonEntryPoint.createMeasure(MeasurementUnit.SQUARE_METERS.getValue(), 150);
		IBinaryMeasure floorMeasurement = (IBinaryMeasure) pythonEntryPoint.createMeasurement(floorSize, "Binary");
		IFloor floorOne = pythonEntryPoint.createFloor(floorMeasurement, 1, FloorType.BASEMENT.getValue(),
				floorMeasurement, "First floor of the building", room, null);

		IFloor floorTwo = pythonEntryPoint.createFloor(floorMeasurement, 2, FloorType.REGULAR.getValue(),
				floorMeasurement, "Second floor of the building", null, openSpace);
		...
		Assert.assertEquals(building.getFloorByNumber(1).getRoomByName(room.getName())
				.getTransducer(tempSensor.getName()).toString(), tempSensor.toString());
	}
	...
	public void TestMultiFloorBuilding() {
		...
		IRoom roomTwo = pythonEntryPoint.createRoom(measurement, "Room 101", RoomType.LIVING_ROOM.getValue(),
				"hei.ies.aos");
		...
		IFloor floorTwo = pythonEntryPoint.createFloor(floorMeasurement, 2, FloorType.REGULAR.getValue(),
				floorMeasurement, "Second floor of the building", null, openSpace);
		IFloor floorThree = pythonEntryPoint.createFloor(floorMeasurement, 3, FloorType.REGULAR.getValue(),
				floorMeasurement, "Third floor of the building", roomTwo, null);
		...
		Assert.assertEquals(building.getFloorByNumber(2).getRooms(null).size(), 0);
		Assert.assertEquals(building.getFloorByNumber(2).getOpenSpaces(null).size(), 1);
		Assert.assertEquals(building.getFloorByNumber(2).getOpenSpaceByName(openSpace.getName())
				.getTransducer(co2Sensor.getName()).getData(null).size(), 20);
		Assert.assertNull(building.getFloorByNumber(3).getRoomByName(room.getName()));
		Assert.assertEquals(building.getFloorByNumber(3).getRoomByName(roomTwo.getName()).toString(),
				roomTwo.toString());
		...
		Assert.assertEquals(building.getFloorByNumber(3).getZoneByName(outerZone.getName()).toString(),
				outerZone.toString());
