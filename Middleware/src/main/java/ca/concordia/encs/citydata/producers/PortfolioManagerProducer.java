...
public class PortfolioManagerProducer extends AbstractProducer<JsonObject> implements IProducer<JsonObject> {
    ...
    private String fetchConsumptionData() {
        try {
            ...
            throw new RuntimeException("Interrupted while fetching meter metadata: "
                    + e.getMessage(), e);
        }
    }
    ...
    public void fetch() {
        if (this.meterId == null) {
            throw new RuntimeException("'meterId' is required. "
                    + "Use PortfolioManagerMetadataProducer with dataType=METER_LIST "
